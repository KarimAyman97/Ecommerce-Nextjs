...
export async function GET(
  request: Request,
  { params }: { params: { category: string; productId: string | number } }
) {
  if (params && params.category && params.productId) {
    const filteredData = TrendingData.filter(
      (item) => item.category == params.category && item.id == params.productId
    );
